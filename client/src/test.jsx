...
const Form = (prop) => {
  ...
  const notify = () => toast.success("You are welcome!", {
    position: "top-center",
    autoClose: 2700,
  ...
  const onSubmit = async (data) => {
    const myRoute = prop.isTrue ? "http://localhost:5500/login" : "http://localhost:5500/register";
    try {
      const response = await axios.post(myRoute, data);
      const tokenVal = response.data.token;
      window.localStorage.setItem("authToken", tokenVal);
      if (response.data.success === true) {
        notify();
        setTimeout(() => {
          navigate("/dashboard");
        }, 3000); // Ensure this timing works as intended
      }
    } catch (error) {
      console.log(error);
    }
